...
window.initAll = function () {
   ...
   * Pipeline animation
   */
  let pipelineContent = select('.pipelines-content');
  if (pipelineContent) {
    new Waypoint({
      element: pipelineContent,
      offset: '80%',
  ...
  /*window.addEventListener('load', () => {
    ...
    if (facilityContainer) {
      ...
      on('click', '#facility-flters li', function (e) {
        ...
        facilityIsotope.on('arrangeComplete', function () {
          AOS.refresh()
        });
      }, true);
    }
  ...
  if (facilityContainer) {
    ...
    setTimeout(() => facilityIsotope.arrange({ filter: '.filter-office' }), 500);
    //setTimeout(() => $('#facility-flters .filter-office').click(), 1000);
  ...
  window.navTo = function (target) {
    ...
    $(`[data-bs-target="#tab-${target}"]`).tab('show');
